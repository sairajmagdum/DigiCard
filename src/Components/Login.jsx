...
function Login() {
  ...
  const auth = useAuth();  // Get the login and user state from context
  const navigate = useNavigate();  // To navigate after login
  const location = useLocation();  // To get the location where the user was trying to go
  const from = location.state?.from?.pathname || "/";  // Default redirect path is home

  // Handle form submission
  const handleLogin = (e) => {
    e.preventDefault();  // Prevent default form submission (page reload)
    
    // Use auth context to login (can be extended with real authentication logic)
    const userData = { email };  // You can add password or any other user info as needed
    auth.login(userData);  // Update the user context
    
    // Redirect user to the previous page or default path
    navigate(from, { replace: true });
  };
        ...
        <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={handleLogin}>
              ...
              <button 
                type="submit" 
                className="cursor-pointer w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 border"
              >
                Sign in
              </button>
              <p className="text-sm font-light text-gray-500 dark:text-gray-400">
